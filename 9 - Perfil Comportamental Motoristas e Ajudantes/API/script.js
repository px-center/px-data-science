...
function showNextQuestion() {
  if (currentMode === "text") {
    if (currentTextIndex < textQuestions.length) {
      // Adiciona a pergunta como bubble (cinza)
      addBubble(textQuestions[currentTextIndex], "question-bubble");
      // Exibe input de texto e botão de envio
      textInputWrapper.style.display = "flex";
      // Oculta controles de áudio
      recordButton.style.display = "none";
      timerDisplay.style.display = "none";
      progressContainer.style.display = "none";
    } else {
      // Passa para modo áudio
      currentMode = "audio";
      showNextQuestion();
    }
  } else {
    ...
    if (currentAudioIndex < audioQuestions.length) {
      addBubble(`<b>Pergunta ${currentAudioIndex + 1}</b><br/><br/>${audioQuestions[currentAudioIndex]}`, "question-bubble");
      // Oculta input de texto
      textInputWrapper.style.display = "none";
      // Exibe controles de áudio
...
// Envio de respostas de texto (opcional: se desejar enviar texto via API, similar ao áudio)
sendTextButton.addEventListener("click", () => {
  ...
  // Validação para o nome completo: deve conter pelo menos duas palavras com no mínimo 2 letras cada
  if (currentTextIndex === 0) {
    const nameParts = answer.split(/\s+/).filter(word => word.length >= 2);
  ...
  // Se for a segunda pergunta (CPF), valida o CPF
  if (currentTextIndex === 1) {
    ...
    cpf = answer.replace(/\D/g, ''); // Armazena apenas os dígitos
  }
  
  // Exibe a resposta do usuário
  addBubble(`${answer}`, "answer-bubble");
...
// Gravação de áudio com transcrição e exibição do player
recordButton.addEventListener("click", async () => {
  ...
  try {
    ...
    mediaRecorder.onstop = () => {
      ...
      // Cria um URL para o áudio gravado
      ...
      // Adiciona o código de verificação sempre, pegando do localStorage
      formData.append("verificationCode", localStorage.getItem("verificationCode"));
      ...
      fetch("https://ds-drivers-interviews-data-acquisition-336884965866.us-east1.run.app", {
        ...
        .then((data) => {
          // Exibe a resposta da API e o player de áudio no balão de resposta
          const playerHTML = `<br><audio controls src="${audioURL}"></audio>`;
          addBubble(`<strong>Resposta ${currentAudioIndex + 1}:</strong><br>${data}${playerHTML}`, "answer-bubble");
          progressBar.style.width = "0%";
          currentAudioIndex++;
          showNextQuestion();
        })

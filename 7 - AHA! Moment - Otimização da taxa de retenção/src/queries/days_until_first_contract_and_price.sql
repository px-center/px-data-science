WITH qualified_freights AS (
    SELECT 
        ...
        FLOOR(EXTRACT(EPOCH FROM (now() - d.qualified_at)) / (60 * 60 * 24 * 7)) AS weeks_from_qualification_to_now
    FROM freights f
    right join  drivers d ON d.id = f.driver_id 
    WHERE f.status = 300
...
SELECT 
    ...
    MAX(weeks_from_qualification_to_now) AS week_max_now,
    ...
    CASE WHEN MAX(weeks_from_qualification_to_now) > 2.6 THEN 'yes' ELSE 'no' END AS crossed_aha_week,
    ...
    CEIL(GREATEST(2.6 - MAX(weeks_from_qualification_to_now), 0) * 7) AS days_to_aha,
    CEIL(GREATEST(2.7 - COUNT(*), 0)) AS contracts_gap_to_aha,
    GREATEST(1519.78 - SUM(price), 0) AS value_gap_to_aha,

    CASE 
        WHEN 
            MAX(weeks_from_qualification_to_now) > 2.6 AND 
            COUNT(*) > 2.7 AND 
            SUM(price) > 1519.78 
        THEN 'yes'
        ELSE 'no'
